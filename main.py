...
numbers = [10, 501, 22, 37, 100, 999, 87, 351] #given List
def is_prime(n):
    if n < 2: #Num Less than 2 is not prime num
        return False
    for i in range(2, int(n**0.5)+1): #Check divisibility from 2 to square root of n
        if n % i == 0:
            return False #Found divisor not prime
    return True #no divisor its prime
#count and collect prime number
prime_count = 0
prime_num = []
...
print("Number\t\tFirst Digit\tLast Digit\tSum") #\t used to display out in table format
for num in test_numbers:
    result = sum_first_last_digit(num)
    num_str = str(num)
    if num_str[0] == '-': #first digit comes with - it takes index value 1
        first_digit = num_str[1]
        last_digit = num_str[-1]
    else:
        first_digit = num_str[0]
        last_digit = num_str[-1]
    print(f"{num:8}\t{first_digit:12}\t{last_digit:10}\t{result}") #\t used to display out in table format
...
common_in_all = set1.intersection(set2, set3) #intersection() method returns a set that contains the similarity between two or more sets.
...
def find_first_non_repeating(numbers):
    ...
    for num in numbers: #counts how many times each number appears.
        if num in frequency:
            frequency[num] += 1
        else:
            frequency[num] = 1
...
print("List\t\t\t\t\tFirst Non-Repeating") #\t to show out in table format
for lst in test_lists:
    result = find_first_non_repeating(lst)
    print(f"{lst}\t\t\t\t{result if result is not None else 'None (all repeat)'}")##\t to show out in table format
...
print(f"Minimum (using min()): {min(test_list)}") #min gets minimum value from a list
print(f"Minimum (using function): {find_minimum_simple(test_list)}")
...
def find_triplet_with_sum(numbers, target_sum):
    ...
    n = len(numbers)
    triplets = []
    for i in range(n): # first index number of the list
...
def has_zero_sum_sublist(numbers):
    ...
    for start in range(n): # loop used to start
        current_sum = 0
        for end in range(start, n): # loop used to end
